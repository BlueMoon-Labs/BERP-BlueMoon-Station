...
/mob/living/carbon/human/update_worn_ears(update_obscured = TRUE)
	...
	if(client && hud_used)
		var/atom/movable/screen/inventory/inv = hud_used.inv_slots[TOBITSHIFT(ITEM_SLOT_EARS_LEFT) + 1] // BLUEMOON EDIT - Extra inventory
		inv.update_icon()
	...
	if(ears)
		...
		if(check_obscured_slots(transparent_protection = TRUE) & ITEM_SLOT_EARS_LEFT) // BLUEMOON EDIT - Extra inventory
			return
